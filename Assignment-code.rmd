---
title: "Group Project"
author: "Team Regression and Beyond"
date: "11/07/2020"
output: pdf_document
---
...
```{r}
data<-read.csv("C://Users//ricky//OneDrive - Bond University//Uni//Stats Learning//StatsAssignmentData.csv")
str(data)
```
...
```{r split}
#Do not have lots of data, so will use metrics that mimic external metrics instead (e.g. BIC and AIC)
```
...
```{r}
#Variable selection through building a tree with tree function
...
##Variable selection for splines

```{r}

lm = lm(Life.expectancy~., data=lifedata)

vars = step(lm, trace=0)
coef(vars)
```
...
```{r}
spline <-gam(Life.expectancy ~ s(Adult.Mortality) + s(Total.expenditure) + s(Diphtheria) + s(HIV.AIDS) + s(thinness.5.9.years) + s(Income.composition.of.resources), data = lifedata)
plot(lifedata$Life.expectancy, fitted(spline))
plot(lifedata$Life.expectancy, resid(spline))
## This is without interaction gam cant have interaction. Choose the one u want.

ggplot(lifedata, aes(lifedata$Life.expectancy, fitted(spline))) +geom_point() + geom_smooth(method = "gam")
plot(spline, se=TRUE)
```

```{r}
##THis is mars. It selects the interaction variables but doesn't make sense. We can however compare it to you interaction variables and see if they are similar.
mars <- earth(Life.expectancy ~., data = lifedata, degree = 3)
summary(mars)$coefficients
evimp(mars)
plot(lifedata$Life.expectancy, fitted(mars))
plot(lifedata$Life.expectancy, resid(mars))
```
```{r}
par(mfrow =c(1,4))
plot(mars, se=TRUE)
...
marsr = mars$rsq
spliner = gamR2(spline)[1,2]
linearr = summary(Linear)$r.squared
mae1=mae(lifedata$Life.expectancy, predict(mars, lifedata))
mae2=mae(lifedata$Life.expectancy, predict(spline, lifedata))
mae3=mae(lifedata$Life.expectancy, predict(Linear, lifedata))


comparison= data.frame(Earth = c(mae1, marsr), Smooth= c(mae2, spliner), Linear= c(mae3, linearr), row.names=c("MAE", "R.squared"))
comparison

#not neccesarily a better model (overfitting), need external measures.
```
